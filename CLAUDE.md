...
Traits are **read from the store's own schema**, not assumed. `traitPayload` filters the payload against `GET /v1/ControlPlane/Stores/{id}/TraitGroups?includeTraits=true` (read once, cached, cleared by `refreshTraitSchema`) because one undeclared key rejects the whole write — dropping the company trait is survivable, losing the name with it is not. `Contact` holds firstName/lastName/phone; the `Wonder` group holds `company`/`role`, which no store ships with, so `probeMemoryTraits` reports what is missing in the HUD and `POST /api/memory/traits` (`ensureTraitGroups`) creates it: POST for a missing group, PATCH to add absent traits (both 202, both merge). Never automatic — it edits the account's schema, which outlives the event. There is no REST delete for a single trait; that is Console-only.
